source $HOME/.vim/autoload/plug.vim  " TODO

if !isdirectory($HOME.'/.config/nvim') | call mkdir($HOME.'/.config/nvim', '', 0700) | endif
if !isdirectory($HOME.'/.config/nvim/undodir') | call mkdir($HOME.'/.config/nvim/undodir', '', 0700) | endif
...
hi DiffAdd     gui=italic     guifg=Green    guibg=none
hi DiffChange  gui=none       guifg=Yellow   guibg=none
hi DiffDelete  gui=bold       guifg=Red      guibg=none
hi DiffText    gui=undercurl  guisp=Yellow   guibg=none
...
set undofile undodir=~/.config/nvim/undodir history=5000  " maintain history
...
highlight ALEWarning gui=underline guisp=blue guibg=none
highlight ALEError   gui=underline guisp=red  guibg=none
